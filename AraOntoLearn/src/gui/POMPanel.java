...
import util.ProbabilityComparator;
...
public class POMPanel extends JPanel {
	...
	private final static String[] m_sTitles = { "Concept", "Instance",
			"Similiarity", "SubclassOf", "InstanceOf", "Relation",
			"SubtopicOf", "Has-a" }; // , "Disjoint" };
	...
	private final static int HAS_A = 7;
	private final static int DISJOINT = 8;

	private POM m_pom;

	private AraOntoLearn m_araOntoLearn;

	private JFrame m_frame;

	public POMPanel(JFrame frame, AraOntoLearn araontolearn, POM pom) {
		m_pom = pom;
		m_frame = frame;
		m_araOntoLearn = araontolearn;
		...
		for (int i = 0; i < m_tables.length; i++) {
			...
			m_tables[i].setDefaultRenderer(Object.class,
					new ColorTableCellRenderer(i));
	...
	private void doChanges() {
		...
		HashMap<POMChange, List<AbstractExplanation>> hmChange2Explanations = m_araOntoLearn
				.getExplanations(changes);
		ChangeDialog cd = new ChangeDialog(m_frame, changes,
				hmChange2Explanations);
		cd.show();
	}
	...
	public void update() {
		for (int i = 0; i < m_tables.length; i++) {
			...
			// (AbstractDataModel)m_tables[i].getModel();
	...
	private AbstractDataModel createDataModel(int iType) {
		switch (iType) {
		case CONCEPTS:
			return new DataModelConcepts();
		case INSTANCES:
			return new DataModelInstances();
		case SUBCLASS_OF:
			return new DataModelSubclassOf();
		case INSTANCE_OF:
			return new DataModelInstanceOf();
		case SUBTOPIC_OF:
			return new DataModelSubtopicOf();
		case RELATIONS:
			return new DataModelRelations();
		case SIMILARITY:
			return new DataModelSimilarity();
		case HAS_A:
			return new DataModelHas_A();
		}
		return null;
	}
	...
	private class DataModelHas_A extends AbstractDataModel {
		...
		public int getRowCount() {
			return getObjects(HAS_A).size();
		}

		public Object getValueAt(int iRow) {
			List objects = getObjects(HAS_A);
			return objects.get(iRow);
		}

		public Object getValueAt(int iRow, int iCol) {
			POMHasARelation object = (POMHasARelation) getValueAt(iRow);
			if (iCol == 0) {
	...
	private List getObjects(int iType) {
		...
		case INSTANCE_OF:
			...
			break;
		case SUBTOPIC_OF:
			objects = m_pom.getObjects(POMSubtopicOfRelation.class);
			break;
		case RELATIONS:
		...
		case SIMILARITY:
			...
			break;
		case HAS_A:
			objects = m_pom.getObjects(POMHasARelation.class);
			break;
		}
		if (objects != null) {
	...
	private class PopupListener extends MouseAdapter {
		...
		private void maybeShowPopup(MouseEvent e) {
			if (e.isPopupTrigger()) {
				...
				if (iRow != -1) {
					AbstractDataModel model = ((POMTable) m_tables[i])
							.getDataModel();
	...
	private class ColorTableCellRenderer extends DefaultTableCellRenderer {
		...
		public Component getTableCellRendererComponent(JTable table,
				Object value, boolean bSelected, boolean bFocus, int iRow,
				int iColumn) {
			Component c = super.getTableCellRendererComponent(table, value,
					bSelected, bFocus, iRow, iColumn);
