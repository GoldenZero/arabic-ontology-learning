...
import persist.PersistenceManager;
import persist.Session;
import pom.POM;
import pom.POMObject;
import pom.POMFactory;
...
import corpus.CorpusFactory;
import corpus.AbstractDocument;
import corpus.DocumentFactory;
import algorithm.*;
import algorithm.normalizer.*;
 ...
 */
public class AraOntoLearn implements CorpusListener, ControllerListener, POMListener {
	...
	public AraOntoLearn( JFrame frame ) {
		m_frame = frame;
		m_araOntoLearn = this;
		this.reset();
	}
	...
	private static void createAndShowGUI() {
		...
		JFrame frame = new JFrame( "Arabic Ontology Learning Frame Work" );
		frame.setDefaultCloseOperation( JFrame.EXIT_ON_CLOSE );
		AraOntoLearn t2o = new AraOntoLearn( frame );
	...
	public void controllerChanged( int iMessage, String sComplex, Class algorithmClass, Class configClass ) {
		...
		{
			// System.out.println( "araontolearn.controllerChanged COMBINER: "+ sComplex +", "+ configClass );
			...
			{
				// System.out.println( "araontolearn.controllerChanged ADD: "+ sComplex +", "+ algorithmClass );
				if( algorithm == null ) {
					try {
				...
				// TODO
				System.out.println( "araOntoLearn.controllerChanged REMOVE: " + sComplex + ", " + algorithmClass );
			}
			else if( iMessage == ControllerListener.AUXILIARY ) 
			{
				// System.out.println( "araOntoLearn.controllerChanged AUXILIARY: "+ sComplex +", "+ algorithmClass +", "+
				// configClass );
	...
	private JMenuBar createMenuBar() {
		...
		m_itemExport.addActionListener( new ActionListener() {
			public void actionPerformed( ActionEvent e ) {
				m_araOntoLearn.doExport();
			}
		} );
		JMenuItem itemExit = new JMenuItem( "Exit" );
		itemExit.addActionListener( new ActionListener() {
			public void actionPerformed( ActionEvent e ) {
				m_araOntoLearn.doExit();
			}
		} );
	...
	private void doAbout() {
		String sText = "Arabic Ontology Learning FrameWork " + m_sVersion
				+ "\n\nMoayad AlSaidALi";
