...
import persist.GateDataStore;
import persist.GateSerialDataStore;
import persist.GateDBDataStore;
...
public class LinguisticPreprocessor implements ChangeObserver, Serializable {
	...
	public void init() throws Exception {
		...
		try {
			Gate.setGateHome(new File(Settings.get(Settings.GATE_DIR)));
		} catch (Exception e) {
		...
		creoleURL = new URL("file:"
				+ System.getProperty("gate.plugins.home", null) + "/ANNIE");
		creoleRegister.addDirectory(creoleURL);
		creoleRegister.registerDirectories(creoleURL);

		creoleURL = new URL("file:"
				+ System.getProperty("gate.plugins.home", null) + "/Stanford");
		creoleRegister.addDirectory(creoleURL);
		creoleRegister.registerDirectories(creoleURL);

		creoleURL = new URL("file:"
				+ System.getProperty("gate.plugins.home", null) + "/Tools");
		creoleRegister.addDirectory(creoleURL);
		creoleRegister.registerDirectories(creoleURL);
/*
		creoleURL = new URL("file:"
				+ System.getProperty("gate.plugins.home", null) + "/Stemmer");
		creoleRegister.addDirectory(creoleURL);
		creoleRegister.registerDirectories(creoleURL);*/

		creoleURL = new URL(
				"file:/E:/newfolder/gate/gate-5.0-beta1-build3048-ALL/plugins/Stanford/");
		creoleRegister.addDirectory(creoleURL);
		creoleRegister.registerDirectories(creoleURL);
		try{
		initApplication();
	...
	private void setLanguage(String sLanguage) {
		...
		m_sLanguageDir = Settings.get(Settings.CREOLE_DIR) + m_sLanguage + "/";
		m_sEncoding = m_sEncoding;
	}

	public void initTransducers() throws MalformedURLException,
			ResourceInstantiationException {
		System.out
				.println("LinguisticPreprocessor: initializing transducers...");
		Transducer mainTransducer = new Transducer();
		mainTransducer.setGrammarURL((new File(m_sLanguageDir
				+ Settings.get(Settings.JAPE_MAIN))).toURL());
		// mainTransducer.setGrammarURL( new URL( "file:"+ Settings.get(
		// Settings.GATE_DIR ) +"main.jape" ) );
		mainTransducer.setEncoding(m_sEncoding);
		/*
		 * Transducer postTransducer = new Transducer();
		 * postTransducer.setGrammarURL( new URL( "file:" + m_sLanguageDir +
		 * "tokeniser/postprocessGerman.jape" ) ); postTransducer.setEncoding(
		 * m_sEncoding );
		 */
	...
	public void initApplication() throws Exception {
		// String sAppFile = Settings.get( Settings.GATE_DIR ) +
		// "application.gate";

		// String sAppFile = m_sLanguageDir + Settings.get(Settings.GATE_APP);

		String sAppFile = "E:\\newfolder\\gate\\gate-5.0-beta1-build3048-ALL\\arabicApp.gapp";

		File appFile = new File(sAppFile);
		if (INIT || !appFile.exists()) {
			System.out.println("LinguisticPreprocessor: creating application "
					+ sAppFile + "...");
			m_application = createApplication(m_sLanguage, sAppFile);
			// appFile = createApplication( m_sLanguage, sAppFile );
		}

		System.out.println("LinguisticPreprocessor: loading application ("
				+ sAppFile + ")...");
		m_application = (SerialAnalyserController) gate.util.persistence.PersistenceManager
	...
	public void addCorpus(String sID, Corpus corpus) throws Exception {
		...
		if (m_hmID2Corpus.get(sID) != null) {
			System.out.println("Preprocessor.addCorpus: corpus " + sID
					+ " already exists!");
			return;
		}
		...
		if (gdstype.equals(DS_SERIAL)) {
			...
			// gds = PersistenceManager.getGateDBDataStore(
			// GateDataStore.POSTGRES );
			gds = new GateDBDataStore(2); // 2 means we want a Storage in a
			// postgresql DB, look in the
			// GateDBDataStore.java class
		}
		// additional database or possibly xml storage possibilities have to be
		// programmed here
		else {
			gds = new GateSerialDataStore(DS_SERIAL);
		}
		// GateSerialDataStore gds = new GateSerialDataStore( Settings.get(
		// Settings.DATASTORE ) );
	...
	protected void doPreprocessing() throws Exception {
		if (m_corpus.hasChangesFor(this)) {
			...
			m_gateCorpus.apply(m_application, m_alTransducers, m_corpus
					.getChangesFor(this));
		} else {
	...
	protected AnnotationSet getAnnotations(AbstractDocument doc, Long lStart,
			Long lEnd) {
		return m_gateCorpus.getAnnotations(doc, lStart, lEnd);
	}

	protected String getDocumentContent(Long lStart, Long lEnd,
			AbstractDocument doc) throws InvalidOffsetException {
	...
	private File createApplication(String sAppFile) throws Exception {
		...
		ProcessingResource pr = (ProcessingResource) Factory.createResource(
				"gate.creole.morph.Morph", Factory.newFeatureMap());
		controller.add(pr);
		...
		System.out.println("linguistic.StopwordDetection");
		FeatureMap params = Factory.newFeatureMap();
		params.put("stopwordsURL", new URL("file:"
				+ Settings.get(Settings.GATE_DIR) + "stopwords.txt"));
		params.put("testUpperCase", new Boolean(true));
		pr = (ProcessingResource) Factory.createResource(
				"linguistic.StopwordDetection", params);
		controller.add(pr);

		File file = new File(sAppFile);
		gate.util.persistence.PersistenceManager.saveObjectToFile(controller,
				file);
	...
	private SerialAnalyserController createApplication(String sLanguage,
			String sAppFile) throws Exception {
		...
		if (sLanguage.equals(Settings.ENGLISH)) {
			...
			System.out.println("linguistic.StopwordDetection");
			FeatureMap params = Factory.newFeatureMap();
			params.put("stopwordsURL", new URL("file:" + m_sLanguageDir
					+ Settings.get(Settings.STOP_FILE)));
			params.put("testUpperCase", new Boolean(true));
			pr = (ProcessingResource) Factory.createResource(
					"linguistic.StopwordDetection", params);
			controller.add(pr);
			...
			for (int i = 0; i < ANNIEConstants.PR_NAMES.length; i++) {
				...
				ProcessingResource pr = (ProcessingResource) Factory
						.createResource(ANNIEConstants.PR_NAMES[i], params);
				if (i == 1) {
					params.put("tokeniserRulesURL", new URL("file:"
							+ m_sLanguageDir
							+ "tokeniser/GermanTokeniser.rules"));
					pr = (ProcessingResource) Factory.createResource(
							ANNIEConstants.PR_NAMES[i], params);
					((DefaultTokeniser) pr).setEncoding(m_sEncoding);
				}
				if (i == 2) {
					String sPlugins = System.getProperty("gate.plugins.home",
							"/c:/HIWI/GATE/plugins");
					params.put("listsURL", new URL("file:" + sPlugins
							+ "/german/resources/gazetteer/lists.def"));
					pr = (ProcessingResource) Factory.createResource(
							ANNIEConstants.PR_NAMES[i], params);
			...
			params.put("lexiconURL", new URL("file:" + m_sLanguageDir
					+ "tagger/german.lexicon"));
			params.put("rulesURL", new URL("file:" + m_sLanguageDir
					+ "tagger/german.full.rules"));
			ProcessingResource pr = (ProcessingResource) Factory
			...
			pr = (ProcessingResource) Factory.createResource(
					"stemmer.SnowballStemmer", params);
			controller.add(pr);
			...
			System.out
					.println("starting application.gate creation for spanish...");
			for (int i = 0; i < ANNIEConstants.PR_NAMES.length; i++) {
				FeatureMap params = Factory.newFeatureMap();
				System.out.println(ANNIEConstants.PR_NAMES[i]);
				ProcessingResource pr = (ProcessingResource) Factory
			...
			System.out.println("linguistic.TreeTagger");
			FeatureMap params = Factory.newFeatureMap();
			String sPlugins = System.getProperty("gate.plugins.home",
					"/c:/HIWI/GATE/plugins");
			params.put("taggerScript", Settings.get(Settings.TAGGER_DIR)
					+ "tag-spanish.bat");
			ProcessingResource prTagger = (ProcessingResource) Factory
					.createResource("linguistic.TreeTagger", params);
			controller.add(prTagger);

			System.out.println("linguistic.Lemmatizer");
			params = Factory.newFeatureMap();
			params.put("lemmatizerScript", Settings.get(Settings.TAGGER_DIR)
					+ "tag-spanish.bat");
			ProcessingResource prLemmatizer = (ProcessingResource) Factory
					.createResource("linguistic.Lemmatizer", params);
			...
			params.put("stopwordsURL", new URL("file:" + m_sLanguageDir
					+ Settings.get(Settings.STOP_FILE)));
			params.put("testUpperCase", new Boolean(true));
			ProcessingResource prStopwords = (ProcessingResource) Factory
					.createResource("linguistic.StopwordDetection", params);
